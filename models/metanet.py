...
class MetaNet(nn.Module):
    ...
    def forward(self, support, query, N, K, Q_total):
        ...
        q_att = self.attention_emb(query, (-1, Q_total, self.hidden_size), use_fast=True)
        ...
        for i in range(Q_total):
            ...
            q = self.basic_emb(
                {'word': query['word'][i:i + 1], 'pos1': query['pos1'][i:i + 1], 'pos2': query['pos2'][i:i + 1],
                 'mask': query['mask'][i:i + 1]}, (self.hidden_size), use_fast=True)
